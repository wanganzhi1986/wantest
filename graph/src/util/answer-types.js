...
const AnswerTypes = {
    ...
    predicate: {
        ...
        createValidatorFunctional: function(predicate, options) {
            ...
            options = _.extend({
                simplify: "required",
                ratio: false,
                forms: AnswerTypes.predicate.defaultForms,
            }, options);
            let acceptableForms;
                ...
                pi: function(text) {
                    ...
                    // 0.5 pi (fallback)
                        ...
                        possibilities = _.reduce(
                            AnswerTypes.predicate.defaultForms.split(
                                    /\s*,\s*/),
                            function(memo, form) {
    ...
    number: {
        ...
        createValidatorFunctional: function(correct, options) {
            return AnswerTypes.predicate.createValidatorFunctional(
                ...AnswerTypes.number.convertToPredicate(
                    correct, options));
        },
    },
        ...
        createValidatorFunctional: function(solution, options) {
            return function(guess) {
                ...
                if (typeof solution === 'string') {
                    solution = AnswerTypes.expression.parseSolution(
                        solution, options);
                }
