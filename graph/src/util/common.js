...
const Util = {
    ...
    firstNumericalParse: function(text) {
        ...
        let first;
        let val = AnswerTypes.predicate.createValidatorFunctional(
            function(ans) {
                first = ans;
                return true;  /* break */
            }, {
    ...
    gridDimensionConfig: function(absTickStep, extent, dimensionConstraint,
                                  gridStep) {
        let scale = Util.scaleFromExtent(extent, dimensionConstraint);
    ...
    getGridStep: function(range, step, boxSize) {
        return _(2).times(function(i) {
            let scale = Util.scaleFromExtent(range[i], boxSize);
            let gridStep = Util.gridStepFromTickStep(step[i], scale);
            return gridStep;
        });
    },
